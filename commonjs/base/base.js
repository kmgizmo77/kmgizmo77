...
var App = Class.extend("",{
	...
	injectNative : function(){
		...
		if(_isIDevice) {
			...
			location.href = "melon://" + data.split("?").join("/");
		...
		if(_isChrome){
			console.info("iOS :" + "melon://" + data.split("?").join("/"));
...
/*
 File Class
 */
var File = App.extend("File",{
                     
    readLocalFile : function(path,callback){
        this.methodname = "readLocalFile";
        this.params = {path:JSON.stringify(path.split("/").join("+")),callback:callback};
        this.execute();
    }
